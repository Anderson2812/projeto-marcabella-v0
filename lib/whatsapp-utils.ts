import { Booking, Professional, ReminderVariationType } from '@/types';
...
export function generateReminderMessageText(
  ...
  const hasDeposit = Boolean(booking.depositRequired || (booking.depositAmount && booking.depositAmount > 0));
  const isDepositPaid = Boolean(booking.depositPaid);
  const paidAmount = isDepositPaid ? (booking.depositAmount || 0) : 0;
  const remainingAmount = Math.max(0, booking.totalPrice - paidAmount);
  ...
  if (variation === 'custom' && customMessage.trim()) {
    return customMessage
      ...
      .replace(/\{dia\}/g, formatDatePtBr(booking.date))
      .replace(/\{hora\}/g, booking.time)
      .replace(/\{horario\}/g, timeText)
      .replace(/\{profissional\}/g, professional.name)
      .replace(/\{endereco\}/g, professional.address || '')
      .replace(/\{total\}/g, `R$ ${booking.totalPrice.toFixed(2)}`)
      .replace(/\{sinal\}/g, hasDeposit ? `R$ ${(booking.depositAmount || 0).toFixed(2)}` : 'Sem sinal')
      .replace(/\{pago\}/g, `R$ ${paidAmount.toFixed(2)}`)
      .replace(/\{restante\}/g, `R$ ${remainingAmount.toFixed(2)}`)
      .replace(/\{saldo\}/g, `R$ ${remainingAmount.toFixed(2)}`)
  ...
  text += `💰 *Valor Total:* R$ ${booking.totalPrice.toFixed(2)}\n`;

  if (hasDeposit) {
    text += `💵 *Sinal de Reserva:* R$ ${(booking.depositAmount || 0).toFixed(2)}\n`;
    if (isDepositPaid) {
      text += `✅ *Valor Pago (Sinal):* R$ ${paidAmount.toFixed(2)} (Confirmado)\n`;
      if (remainingAmount > 0) {
        text += `💳 *Valor a ser pago no horário:* R$ ${remainingAmount.toFixed(2)}\n`;
      } else {
        text += `✨ *Valor a ser pago no horário:* R$ 0,00 (Totalmente quitado)\n`;
      }
    } else {
      text += `⏳ *Status do Sinal:* Pendente de confirmação\n`;
      text += `💳 *Valor a ser pago no horário:* R$ ${booking.totalPrice.toFixed(2)}\n`;
    }
  } else {
    text += `💳 *Valor a ser pago no horário:* R$ ${booking.totalPrice.toFixed(2)} (no local)\n`;
  }
...
export function generateBookingConfirmedWhatsAppUrl(booking: Booking, professional: Professional): string {
  const text =
    ...
    (booking.payOnArrival ? `💵 *Forma de Pagamento:* Pagar na hora no atendimento (R$ ${booking.totalPrice.toFixed(2)})\n` : (booking.depositRequired ? ` *Sinal Pix:* R$ ${booking.depositAmount.toFixed(2)} (Confirmado)\n` : '')) +
...
export function generateClientPreBookingWhatsAppUrl(booking: Booking, professional: Professional): string {
  const text =
    `Olá *${professional.name}*! 👋 Acabei de solicitar um agendamento na sua agenda Marcabella:\n\n` +
...
export function generateCancellationWhatsAppUrl(
  booking: Booking,
  professional: Professional,
  reason?: string
): string {
  const text =
    `Olá *${booking.clientName}*, aqui é a *${professional.name}*.\n\n` +
    `Passando para avisar que o seu agendamento para o procedimento *${booking.serviceName}* no dia *${formatDatePtBr(booking.date)} às ${booking.time}* precisou ser cancelado.\n\n` +
    (reason ? `📌 *Motivo informado:* ${reason}\n\n` : '') +
    `Pedimos desculpas pelo imprevisto. Fique à vontade para me responder aqui para reagendarmos em um novo horário conveniente para você! 😊`;
...
export function generateShareScheduleLinkUrl(professional: Professional, baseUrl: string): string {
  const cleanLink = `${baseUrl}/${professional.slug}`;
  const text = `✨ Olá! Agende seu horário com praticidade pelo nosso link direto Marcabella:\n\n🔗 ${cleanLink}`;
  return `https://wa.me/?text=${encodeURIComponent(text)}`;
}

export function generateProfessionalNotificationWhatsAppUrl(
  booking: Booking,
  professional: Professional,
  baseUrl?: string
): string {
  return `https://wa.me/?text=${encodeURIComponent('Novo agendamento!')}`;
}
